...
def cifar10_dataloaders(
    ...
    train_set = CIFAR10(data_dir, train=True, transform=train_transform, download=True)
    retain_set_train = CIFAR10(data_dir, train=True, transform=train_transform, download=True)
    test_set = CIFAR10(data_dir, train=False, transform=test_transform, download=True)
    retain_set_test = CIFAR10(data_dir, train=False, transform=test_transform, download=True)
    forget_set = CIFAR10(data_dir, train=False, transform=test_transform, download=True)

    train_set.targets = np.array(train_set.targets)
    test_set.targets = np.array(test_set.targets)
    retain_set_train.targets = np.array(retain_set_train.targets)
    retain_set_test.targets = np.array(retain_set_test.targets)
    forget_set.targets = np.array(forget_set.targets)
    ...
    if class_to_replace is not None:
        ...
        if num_indexes_to_replace is None or num_indexes_to_replace == 4500:
            test_set.data = test_set.data[test_set.targets != class_to_replace]
            test_set.targets = test_set.targets[test_set.targets != class_to_replace]
            forget_set.data = forget_set.data[forget_set.targets == class_to_replace]
            forget_set.targets = forget_set.targets[forget_set.targets == class_to_replace]
            retain_set_test.data = retain_set_test.data[retain_set_test.targets != class_to_replace]
            retain_set_test.targets = retain_set_test.targets[retain_set_test.targets != class_to_replace]
    if indexes_to_replace is not None:
        replace_indexes(
            dataset=train_set,
            indexes=indexes_to_replace,
    ...
    return train_loader, val_loader, test_loader, retain_train_loader, retain_test_loader, forget_loader
